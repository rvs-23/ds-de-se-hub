...
train['discussion'] = train.url.isna().astype('int')
...
train['discussion_hn'] = ((train.url.isna()) & (train.title.str.contains(' HN: '))).astype('int')
train.groupby('discussion_hn').popular.mean()


# ~ means 'not', thus it reverses True and False
train['discussion_other'] = ((train.url.isna()) & (~train.title.str.contains(' HN: '))).astype('int')
...
train.groupby('popular').num_comments.describe()
...
def make_features(df):
    ...
    df['discussion_hn'] = ((df.url.isna()) & (df.title.str.contains(' HN: '))).astype('int')
    df['discussion_other'] = ((df.url.isna()) & (~df.title.str.contains(' HN: '))).astype('int')
    ...
    df['author_popularity'] = df.author_popularity.fillna(0.2)
...
# 
#     - **Note:** An AUC of 1.0 represents a perfect model, and an AUC of 0.5 represents random guessing. You can think of 0.5 as the AUC of the "null model". (My [blog post and video](https://www.dataschool.io/roc-curves-and-auc-explained/) explain AUC in more depth.)
...
# use 'liblinear' solver because it supports both L1 and L2 regularization
